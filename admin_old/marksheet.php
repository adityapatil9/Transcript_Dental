...
<?php
	...
	?>
	<center class="no-print"> <input type="button" class="button" value="Print Transcripts" onclick="window.open('printing.php');"></input></center>
    <br>
	...
	for($i=0; $i<$count_student_total; $i++)
	{
		...
		else
		{
			...
			echo $arr[$yr++].'<br><br>';
			echo $branch_nm.'</b><br><br></h4></center>';
		}
		
		echo '<b><h4><center>SEMSTER - '.$sem_roman[$sem[$i]].'</b></h4></center>';
		if($i%2==0)
		{
			echo '</h4><p><b>STUDENT\'S NAME : </b>'.$name,'</p>';	
		}
		echo "<div ><div style='float:left;display:inline-block'> <b>UNIVERISTY OF MUMBAI SEAT NO : </b>".$seat[$i]."</div><div style='float:right;display:inline-block'><b> DATE OF PASSING : </b>".$year[$i].'</div></div>';
