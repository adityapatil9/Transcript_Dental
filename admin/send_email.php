...
//session_start();
				...
				$roll = $_POST['roll'];
				$sql ="SELECT * FROM current_status WHERE status=2 AND roll='$roll' ";
				$res =mysqli_query($connect,$sql);
				if(mysqli_num_rows($res) == 0)
				{
					echo '<script>alert ("Invalid Status... Please check Roll Number"); window.location="index.php"</script>';
					
				}
				else
				{
				$sql_query_email = 'SELECT * FROM `student` WHERE roll="'.$roll.'"';
				$result_email = mysqli_query($connect,$sql_query_email);
				...
				$Content = "<a href='http://103.101.233.162:81/transcript/Documents/Transcript/Verified/$roll/transcript.pdf'>Download Transcript</a>";
				$Content.= "<br><br> <a href='http://103.101.233.162:81/transcript/Documents/Transcript/Verified/$roll/coverletter.pdf'>Download Coverletter</a>";
				$Content.= "<br><br> Install this certificate for digital sign : http://103.101.233.162:81/transcript/Documents/Transcript/Verified/certificate_principal.cer";
				$to = "transcript.rait@gmail.com,".$email_student.",paprn@rait.ac.in";
